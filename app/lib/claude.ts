...
STEP 4. 양쪽 삼각근(어깨 근육) 가장 바깥쪽 끝 사이의 픽셀 거리를 측정하고 cm로 변환하세요. 옷 실루엣 경계선이 아니라 어깨 근육이 가장 바깥으로 돌출된 지점 기준입니다. 옷을 입고 있어도 근육 위치를 추정해서 측정하세요. 견봉(뼈) 기준이 아닙니다.
...
STEP 9. 오른팔 삼각근 끝에서 손목까지 팔의 윤곽을 따라 경로 길이를 측정하고 cm로 변환하세요. 직선 거리가 아니라 팔이 구부러진 경우 그 곡선을 따라 재야 합니다. 민소매나 나체인 경우 팔 전체가 보이므로 더 정확하게 측정 가능합니다. 30cm 미만으로 측정되면 오류이므로 키/몸무게 비율(키 × 0.35)로 재보정하세요.
STEP 10. 총장을 추정하세요. 옷을 입고 있으면 어깨 봉제선 끝에서 옷 밑단까지의 수직 거리를 측정하세요. 벨트나 벨트 버클이 보이면 이를 허리 위치 기준점으로 활용하세요. 상의탈의 상태면 어깨 끝에서 배꼽~골반 사이를 기준으로 추정하세요. 반드시 옷의 아랫단 끝 또는 신체 기준점을 잡아야 하며, 소매 끝이나 팔 끝과 절대 혼동하지 마세요. 팔을 내렸을 때 소매가 밑단 높이까지 내려와도 밑단은 몸통 기준이에요. 상의 기준 55~75cm, 아우터/코트 기준 70~120cm 범위를 벗어나면 재측정하세요.
...
export async function createFinalFitComment({
  ...
  const instruction = isProfileFallback
    ...
    : "추천 사이즈와 그 이유를 2~3문장으로 설명해줘. 반드시 어깨/가슴/소매 등 실제 측정된 수치와 판정 결과(TIGHT/FIT/루즈 등)만 근거로 써. 제품 스타일, 디자인 의도, 핏 철학 등 측정값에 없는 내용은 절대 추측해서 쓰지 마. 기술적인 측정 방법(픽셀, mm 등)도 언급하지 마.";
